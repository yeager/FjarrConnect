FjärrConnect 0.2.12 — verified SFTP transfer resume.
...
Download `FjarrConnect-0.2.12-macOS-arm64.zip` for Apple Silicon or `FjarrConnect-0.2.12-macOS-x86_64.zip` for Intel. Each app contains only its target architecture and requires macOS 14 or later. Unzip and move FjärrConnect to Applications. `SHA256SUMS.txt` contains both download checksums.
