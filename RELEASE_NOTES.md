...
Known limitations: VNC clipboard supports text only; clipboard file copying is not implemented. Multi-monitor RDP, USB redirection, and VNC VeNCrypt/TLS or RSA-AES authentication are not implemented. RDP printer, smart-card, audio, microphone and RemoteApp support is opt-in and requires server/runtime support. See the README for protocol details.
