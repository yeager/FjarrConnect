...
extension VNCCAFramebufferView {
	...
	func handleKeyDown(with event: NSEvent?) {
		...
		// Some keyboard layouts use Shift or Option to produce a character that
		// RFB can already represent directly (for example Swedish Option+2
		// produces "@"). RFB servers still apply held modifiers to character
		// keysyms, so sending both would apply the layout transformation twice.
		// Release layout modifiers only for this character event, then restore
		...
		if !temporarilyReleasedModifiers.isEmpty {
			temporarilyReleasedModifiersByKey[physicalKeyCode] = temporarilyReleasedModifiers
		}
	...
	private func layoutModifiersToRelease(for event: NSEvent) -> [(key: VNCKeyCode, flag: NSEvent.ModifierFlags)] {
		...
		var modifiers: [(key: VNCKeyCode, flag: NSEvent.ModifierFlags)] = []
		if event.modifierFlags.contains(.leftShift) {
			modifiers.append((.shift, .leftShift))
		}
		if event.modifierFlags.contains(.rightShift) {
			modifiers.append((.rightShift, .rightShift))
		}
		if event.modifierFlags.contains(.leftOption) {
			modifiers.append((.option, .leftOption))
		}
	...
	func handleKeyUp(with event: NSEvent?) {
		...
		if let temporarilyReleasedModifiers = temporarilyReleasedModifiersByKey.removeValue(forKey: CGKeyCode(event.keyCode)) {
			for modifier in temporarilyReleasedModifiers where event.modifierFlags.contains(modifier.flag) {
				connection.keyDown(modifier.key)
			}
		}
	}
	...
	func releasePressedKeys() {
		...
		temporarilyReleasedModifiersByKey.removeAll()
