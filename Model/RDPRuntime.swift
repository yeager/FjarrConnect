...
final class RDPRuntime {
    ...
    init?(path: String) {
        ...
        guard let abi = function("fc_rdp_abi", (@convention(c) () -> UInt32).self), abi() == 1,
              ...
              let failure = function("fc_rdp_failure", Status.self) else { dlclose(library); return nil }
        self.library = library
        self.create = create; self.start = start; self.stop = stop
        self.activate = activate; self.status = status; self.error = error; self.failure = failure
